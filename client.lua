...
Citizen.CreateThread( function()
	WarMenu.CreateMenu('id_boat', 'Tienda de barcos')
	while true do
...
RegisterCommand("barco", function(source, args, raw)
    if recentlySpawned <= 0 then
				...
				TriggerEvent('chat:systemMessage', 'Espera ' .. recentlySpawned .. 's antes de sacar tu barco.')
				TriggerEvent('chat:addMessage', {
					color = { 171, 59, 36 },
					multiline = true,
					args = {"Anti-Spam", 'Espera ' .. recentlySpawned .. 's antes de sacar tu barco.'}
				})
			end
   end)
